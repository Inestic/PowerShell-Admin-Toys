...
{	
	...
	$SenderTextBox, $RecipientTextBox, $SubjectTextBox | ForEach-Object -Process {
		...
		{
			$TrackingCommand += " -{0} ""{1}"""-f $_.Tag, $_.Text
		}
	}

	if ($EventIdTextBox.SelectedIndex -ne 0)
	{
		$TrackingCommand += " -{0} {1}"-f $EventIdTextBox.Tag, $MailEventId[$EventIdTextBox.SelectedIndex]
	}
...
function Export-MessageData
{
	...
	{
		$FoundMailDataGrid.Items | ConvertTo-Csv -Delimiter "*" -NoTypeInformation | Set-Content -Path $SaveFileDialog.FileName -Encoding Default -Force
	}
}
#endregion Functions
